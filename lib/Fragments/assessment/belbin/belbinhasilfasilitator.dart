...
import 'package:safe/Models/HasilBelbinAllRole.dart';
...
class BelbinHasillFasilitator extends StatefulWidget {
  const BelbinHasillFasilitator({Key? key, required this.tkg})
      : super(key: key);

  final TestKelasGrouping tkg;
  @override
  State<BelbinHasillFasilitator> createState() =>
      _BelbinHasillFasilitatorState();
}

class _BelbinHasillFasilitatorState extends State<BelbinHasillFasilitator> {
  ...
  List<HasilBelbinAllRole> _hasil = <HasilBelbinAllRole>[];
  List<HasilBelbinAllRole> _hasilfiltered = <HasilBelbinAllRole>[];
  late HasilBelbinAllRoleDataSource
  ...
  Future<void> _getHasilData() async {
    ...
    var req = await apiService.getSkorBelbinAllPeserta(
        user: nip,
        idkelas: tkg.idkelas,
    ...
    setState(() {
      ...
      _hasilDataSource = HasilBelbinAllRoleDataSource(
  ...
  void searchPeserta() {
    List<HasilBelbinAllRole> tasmnt = _hasil;
    setState(() {
      _hasilfiltered = tasmnt
          .where((element) =>
...
class HasilBelbinAllRoleDataSource extends DataGridSource {
  /// Creates the employee data source class with required details.
  ApiService apiService = Get.find(tag: 'apiserv1');
  List<DataGridRow> _hasilData = <DataGridRow>[];
  String nip = "";
  late TestKelasGrouping tkg;
  late List<HasilBelbinAllRole> hasilData;
  HasilBelbinAllRoleDataSource(
      {required List<HasilBelbinAllRole> hasilData1,
  ...
  void updateDataGridRows() {
    _hasilData = hasilData
        .map<DataGridRow>((HasilBelbinAllRole e) =>
            DataGridRow(cells: <DataGridCell>[
              ...
              DataGridCell<String>(columnName: 'CO', value: e.CO),
              DataGridCell<String>(columnName: 'TW', value: e.TW),
              DataGridCell<String>(columnName: 'RI', value: e.RI),
              DataGridCell<String>(columnName: 'PL', value: e.PL),
              DataGridCell<String>(columnName: 'ME', value: e.ME),
              DataGridCell<String>(columnName: 'SP', value: e.SP),
              DataGridCell<String>(columnName: 'SH', value: e.SH),
              DataGridCell<String>(columnName: 'IMP', value: e.IMP),
              DataGridCell<String>(columnName: 'CF', value: e.CF),
            ]))
        .toList();
  }
                    ...
                    return MaterialButton(
                        ...
                        onPressed: () async {
                          ...
                          if (json1["respon"] == "ya") {
                            ...
                            hasilData = <HasilBelbinAllRole>[];
                            if (json.decode(req.body)['data'] != null) {
                              for (int i = 0;
                              i < json.decode(req.body)['data'].length;
                              i++) {
                                hasilData.add(HasilBelbinAllRole.fromJSON(
                                    json.decode(req.body)['data'][i]));
                              }
                            }
                            updateDataGridRows();
