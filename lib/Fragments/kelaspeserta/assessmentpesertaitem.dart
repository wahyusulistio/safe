...
import 'package:safe/Fragments/kelaspeserta/dialogjoinkelompok.dart';
...
class _AssessmentPesertaItemState extends State<AssessmentPesertaItem> {
  ...
  Future<void> getHasilBelbin() async {
    var req = await apiService.cekBelbin(
        user: nip,
  ...
  Future<void> getHasilSosiometri() async {
    var req = await apiService.cekSosiometriPeserta(
        user: nip,
        idkelas: indasmnt.idkelas,
        iddiklat: indasmnt.iddiklat,
        idmatadiklat: indasmnt.idmatadiklat,
        ...
        if (indasmnt.jenistes == "0") {
          ...
          if (cekSosiometriPeserta) {
            ...
            Get.defaultDialog(
              content: MyDialogInfo(
                  info: "Maaf Anda sudah mengisi tool ini sebelumnya."),
              titleStyle: TextStyle(fontSize: 0),
            );
          } else {
            //cek group peserta
            var isingroup = await apiService.getKelompokPesertaGrouping2(
                user: nip, idgrouping: indasmnt.idgrouping, nippeserta: nip);

            List<Group> tgroup = <Group>[];
            if (json.decode(isingroup.body)['data'] != null) {
              for (int i = 0;
                  i < json.decode(isingroup.body)['data'].length;
                  i++) {
                //print("data:"+json.decode(req.body)['data'][i]);
                tgroup.add(
                    Group.fromJSON(json.decode(isingroup.body)['data'][i]));
              }
            }

            if (tgroup.length > 0) {
              //jika sudah trdaftar d grup
              gotoLembarJawabanSosiometri();
            } else {
              await Get.defaultDialog(
                content: MyDialogInfo(
                    info:
                        "Anda harus terdaftar dalam grup untuk mengerjakan aktivitas ini!"),
                titleStyle: TextStyle(fontSize: 0),
              );
              //popup pilihan kelompok
              var data=await showDialog(
                  context: navigatorKey.currentContext!,
                  builder: (context) {
                    return AlertDialog(
                      content: SingleChildScrollView(
                        child: MyDialogJoinKelompok(
              ...
              if(data=="ok")
                {
                  await Get.defaultDialog(
                    content: MyDialogInfo(info: "Anda telah dimasukkan ke dalam kelompok dan akan dialihkan ke halaman kelompok Anda."),
                    titleStyle: TextStyle(fontSize: 0),
                  );
                  gotoLembarJawabanSosiometri();
                }
          ...
          if (cekBelbinPeserta == false) {
            ...
            for (int p = 0; p < pertanyaanbelbin.length; p++) {
              tjawab =
                  await getPilihanJawaban(pertanyaanbelbin[p].idpertanyaan);
          ...
          Get.to(() => MBTIIntro(
                idtest: indasmnt.idtest,
                nip: nip,
                tkg: TestKelasGrouping(
                    idtest: indasmnt.idtest,
                    idgrouping: indasmnt.idgrouping,
                    iddiklat: indasmnt.iddiklat,
                    idmatadiklat: indasmnt.idmatadiklat,
                    idkelas: indasmnt.idkelas,
                    inclass: indasmnt.inclass,
                    ingroup: indasmnt.ingroup,
                    jenistes: "2",
                    namates: indasmnt.namates,
                    status: "1"),
              ));
  ...
  Future<void> gotoLembarJawabanSosiometri() async {
  ...
  Future<List<Pertanyaan>> getSoalTest(String idtest) async {
    var req = await apiService.getSoalTest(user: nip, idtest: idtest);
  ...
  Future<List<PilihanJawaban>> getPilihanJawaban(String idpertanyaan) async {
    var req = await apiService.getPilihanJawaban(
        user: nip, idpertanyaan: idpertanyaan);
  ...
  Future<List<Peserta>> getPesertaLainDiGrup(String idtest) async {
    var req = await apiService.getPesertaLainDiGroup(
        user: nip, idgrouping: indasmnt.idgrouping);

    print("body peserta grup " + req.body);
